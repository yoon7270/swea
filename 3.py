...
while True:
    ...
    targetBalls = []
    print(f'order {order}')
    for idx , ball in zip([i for i in range(len(balls))],balls):
        if idx == 0: continue
        if ball[0] != -1 and ball[1] != -1:
            if order == 1 and idx in [1,3,5]:
                targetBalls.append(ball)
            elif order != 1 and idx in [2,4,5]:
                targetBalls.append(ball)

    test = random.random() if len(targetBalls) == 3 else 0

    print(targetBalls)

    # targetBall_x, targetBall_y: 목적구의 X, Y좌표를 나타내기 위해 사용한 변수
    targetBall_x = targetBalls[0][0]
    targetBall_y = targetBalls[0][1]
    ...
    angle = angle + test
    power = distance * (0.5 + test)

    # 기억상 코드
    # dx , dy = whiteBall_x - targetBall_x , whiteBall_y - targetBall_y
    # d = math.hypot(dx,dy)
    # ax , ay = targetBall_x - dx / d - 5.73 , targetBall_y - dy / d - 5.73
    #
    # angle = math.dgres(math.atan2(ax - whiteBall_x , ay - whiteBall_y))
